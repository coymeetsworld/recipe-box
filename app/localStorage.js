...
export const loadState = () => {
	try {
		const serializedState = localStorage.getItem('_coymeetsworld_recipes') || JSON.stringify(exampleRecipes);

		return JSON.parse(serializedState).map((recipe) => {
...
export const saveState = (state) => {
	...
	try {
		...
		localStorage.setItem('_coymeetsworld_recipes', serializedState);
	} catch (err) {
		console.log("Caught error in saveState(): " + err);
	}
